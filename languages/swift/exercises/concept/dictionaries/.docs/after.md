[Dictionaries][dictionaries] are one of Swift's three primary collection types. Dictionaries store mappings between _keys_ which are elements of one type and _values_ which are elements of another type (possibly the same type as that of the keys. Unlike arrays, dictionaries are unordered collections. There is no way to, for example, add an entry to the front or back of the dictionary.
...
Note that not all types can be used as dictionary keys, they must have a special property called being Hashable. However, all of the basic Swift types have this property, and it can be added to other types.
