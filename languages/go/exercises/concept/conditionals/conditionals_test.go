...
func TestBlackjack(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := Blackjack(tt.hand.card1, tt.hand.card2); got != tt.want {
				t.Errorf("Blackjack(%s, %s) = %t, want %t", tt.hand.card1, tt.hand.card2, got, tt.want)
			}
		})
	}
}
...
func TestFirstTurn(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "score of 16 with five for dealer",
		...
		{
			name:   "score of 15 with two for dealer",
		...
		{
			name:   "score of 14 with three for dealer",
