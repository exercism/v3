...
Bindings are defined using the `let` keyword. Specifying their type is optional for most bindings, as F#'s _type inference_ can usually infer their type based on their value.
