There are three floating-point types in C#: `float`, `double` and `decimal`. The most commonly used type is `double`, whereas `decimal` is normally used when working with monetary data. A `double` is defined as `2.45` or `2.45d`, a `float` as `2.45f` and a decimal as `2.45m`.
